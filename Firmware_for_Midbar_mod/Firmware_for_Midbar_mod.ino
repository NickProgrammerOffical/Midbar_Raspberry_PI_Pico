...
Adafruit_ST7735 gfx = Adafruit_ST7735(TFT_CS1, TFT_DC1, TFT_RST1);
...
String kderalgs = "UIYu8PN6p656XOFLyATgG6SByNAk4Jv46Qj1KGmH0E3On";
int numofkincr = 651;
byte hmackey[] = {"2qve3whd558vvos8ZZf4q5KdSi9263g8kW4Yz50O57usnTsk6De5gDJESy3kg4EaYU3pOzOTWpIifG8X1sd9l6rVa3jzSTSt6t1Oydrfjn9M5Bhnkg3yo2gFsdFPeH0FIIRj92p1qSZv6b8"};
byte des_key[] = {
0x7f,0xbd,0x4c,0xa0,0xd8,0x37,0xc6,0xc0,
0x34,0xaf,0xb5,0x8a,0x5a,0x90,0x64,0xc5,
0x12,0xe1,0xd9,0x21,0x99,0xfa,0x4e,0xf9
};
uint8_t AES_key[32] = {
0xe1,0xaf,0xa5,0xae,
0xde,0xf2,0x6e,0x4f,
...
void encdr_and_keyb_input() {
  ...
  while (finish_input == false) {
    ...
    if (b_button.press()) {
      ...
      //Serial.println(keyboard_input);
      gfx.fillRect(0, 40, 160, 128, 0x0000);
      //Serial.println(keyboard_input);
      disp();
    }
    delayMicroseconds(400);
...
void enter_username_for_login(int chsn_slot, String entered_title) {
  act = true;
  clear_variables();
  set_stuff_for_input("Enter Usrname");
  encdr_and_keyb_input();
  if (act == true) {
...
void enter_password_for_login(int chsn_slot, String entered_title, String entered_username) {
  act = true;
  clear_variables();
  set_stuff_for_input("Enter Pssword");
  encdr_and_keyb_input();
  if (act == true) {
...
void enter_cardholder_for_credit_card(int chsn_slot, String entered_title) {
  act = true;
  clear_variables();
  set_stuff_for_input("Cardholder Name");
  encdr_and_keyb_input();
  if (act == true) {
...
void enter_card_number_for_credit_card(int chsn_slot, String entered_title, String entered_cardholder) {
  act = true;
  clear_variables();
  set_stuff_for_input("Card Number");
  encdr_and_keyb_input();
  if (act == true) {
...
void enter_expiry_for_credit_card(int chsn_slot, String entered_title, String entered_cardholder, String entered_card_number) {
  act = true;
  clear_variables();
  set_stuff_for_input("Expiration Date");
  encdr_and_keyb_input();
  if (act == true) {
...
void set_pass() {
  ...
  gfx.setTextColor(0xffff);
  disp_centered_text("Setting Master Password", 10);
  disp_centered_text("Please wait", 45);
  disp_centered_text("for a while", 100);
  //Serial.println(keyboard_input);
  ...
  */
  gfx.setTextColor(0xffff);
  disp_centered_text("Master Password Set", 5);
  disp_centered_text("Successfully", 25);
  disp_centered_text("Press Enter", 45);
  disp_centered_text("or Quad-click", 65);
  disp_centered_text("the encoder button", 85);
  disp_centered_text("to continue", 105);
  bool cont1 = true;
  while (cont1 == true) {
    encoder_button.tick();
    if (encoder_button.hasClicks(4))
...
void unlock_midbar() {
  ...
  */
  gfx.setTextSize(1);
  disp_centered_text("Unlocking Midbar", 5);
  disp_centered_text("Please wait", 25);
  disp_centered_text("for a while", 45);
  //Serial.println(keyboard_input);
  String bck = keyboard_input;
  modify_keys();
  keyboard_input = bck;
  ...
  */
  if (next_act == true) {
    gfx.setTextSize(1);
    disp_centered_text("Midbar unlocked", 5);
    disp_centered_text("successfully", 25);
    disp_centered_text("Press Enter", 45);
    disp_centered_text("or Quad-click", 65);
    disp_centered_text("the encoder button", 85);
    ...
    gfx.setTextSize(1);
    gfx.setTextColor(five_six_five_red_color);
    disp_centered_text("Wrong Password!", 5);
    gfx.setTextColor(0xffff);
    disp_centered_text("Please reboot", 40);
    disp_centered_text("the device", 60);
    disp_centered_text("and try again", 80);
    for (;;)
      delay(1000);
  }
}
...
void call_main_menu() {
  ...
  for (int i = 0; i < 160; i++) {
    for (int j = 0; j < 41; j++) {
      gfx.drawPixel(i, j, handwritten_midbar[i][j]);
    }
  }
  curr_key = 0;
  main_menu(curr_key);
...
void main_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("Logins", 45);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Credit Cards", 56);
    disp_centered_text("Notes", 67);
    disp_centered_text("Phone Numbers", 78);
    disp_centered_text("Encryption Algorithms", 89);
    disp_centered_text("Hash Functions", 100);
    disp_centered_text("Factory Reset", 111);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Logins", 45);
...
void input_source_for_data_in_flash_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("Encoder + Keyboard", 40);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Serial Terminal", 53);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Encoder + Keyboard", 40);
...
void action_for_data_in_flash_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("Add", 40);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Edit", 60);
    disp_centered_text("Delete", 80);
    disp_centered_text("View", 100);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Add", 40);
...
void what_to_do_with_encr_alg_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("Encrypt String", 40);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Decrypt String", 53);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Encrypt String", 40);
...
void encryption_algorithms_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("3DES+AES+Blfish+Serp CBC", 40);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("Blowfish+AES+Serp+AES", 53);
    disp_centered_text("AES+Serpent+AES", 66);
    disp_centered_text("Blowfish+Serpent", 79);
    disp_centered_text("AES+Serpent", 92);
    disp_centered_text("Serpent", 105);
    disp_centered_text("Triple DES", 118);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("3DES+AES+Blfish+Serp CBC", 40);
...
void hash_functions_menu(int curr_pos) {
  ...
  if (curr_pos == 0) {
    gfx.setTextColor(0xffff);
    disp_centered_text("SHA-256", 40);
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("SHA-512", 53);
  }
  if (curr_pos == 1) {
    gfx.setTextColor(current_inact_clr);
    disp_centered_text("SHA-256", 40);
...
void setup(void) {
  ...
  gfx.fillScreen(0x0000);
  gfx.setRotation(1);
  int qw = 200 + rnd_whitened() % 100;
  for (int i = 0; i < qw; i++){
    trash += rnd_whitened() % (i + 10);
  }
    chosen_lock_screen = (trash ^ rnd_whitened()) % 8;
  if (chosen_lock_screen == 0) {
    for (int i = 0; i < 160; i++) {
      for (int j = 0; j < 128; j++) {
        gfx.drawPixel(i, j, GB[i][j]);
      }
    }
  }
  ...
  if (chosen_lock_screen == 5) {
    for (int i = 0; i < 160; i++) {
      for (int j = 0; j < 128; j++) {
        gfx.drawPixel(i, j, hungbridge[i][j]);
      }
    }
  }
